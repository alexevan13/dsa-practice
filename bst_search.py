...
def generate_bst(arr):
    ...
    mid = len(arr) // 2
    return TreeNode(
        val=arr[mid], left=generate_bst(arr[:mid]), right=generate_bst(arr[mid + 1:])
    )
...
def element_exists_in_bst_iterative(root, elem):
    stack = [root]
    while len(stack) > 0:
        head = stack.pop()
        if head:
...
elem = 2
print(f"{elem} exists in given bst = {element_exists_in_bst_iterative(bst, elem)}")
